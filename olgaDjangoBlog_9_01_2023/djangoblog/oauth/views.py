...
def get_redirecturl(request):
    ...
    if p.netloc:
        ...
        if not p.netloc.replace('www.', '') == site.replace('www.', ''):
            logger.info('незаконный URL:' + nexturl)
            return "/"
    return nexturl
...
def emailconfirm(request, id, sign):
    ...
    site = get_current_site().domain
    content = '''
     <p>Поздравляем, вы успешно привязали свой 
     почтовый ящик, можете использовать{type}
     Зайдите и войдите на этот сайт напрямую без пароля.
     Вы можете продолжать обращать внимание на этот сайт, 
     адрес</p>
    ...
    send_email(emailto=[oauthuser.email, ], title='Поздравляем с успешной привязкой!', content=content)
...
class RequireEmailView(FormView):
    ...
    def form_valid(self, form):
        ...
        content = """
                <p>Пожалуйста, нажмите на ссылку ниже, 
                чтобы связать свой адрес электронной почты</p>
                <a href="{url}" rel="bookmark">{url}</a>
                Еще раз спасибо!
                <br />
                Если ссылка выше не работает, 
                ...
                {url}
                """.format(url=url)
        send_email(emailto=[email, ], title='Привяжите свою электронную почту',
                   content=content)
        url = reverse('oauth:bindsuccess', kwargs={
            'oauthid': oauthid
        })
...
def bindsuccess(request, oauthid):
    ...
    oauthuser = get_object_or_404(OAuthUser, pk=oauthid)
    if type == 'email':
        title = 'успешно связать'
        content = "Поздравляем," \
                  "вы в одном шаге от успешной привязки." \
                  "Пожалуйста, войдите в свой почтовый ящик," \
                  "чтобы проверить электронную почту," \
                  "чтобы завершить привязку, спасибо." \
                  "Пожалуйста, войдите в свой почтовый ящик," \
                  "чтобы проверить электронную почту," \
                  "чтобы завершить привязку, спасибо."
    else:
        title = 'успешно связать'
        content = "Поздравляем с успешной привязкой," \
                  "вы можете использовать ее позже" \
                  "{type}Зайдите и войдите на этот сайт" \
                  "напрямую без пароля.".format(type=oauthuser.type)
