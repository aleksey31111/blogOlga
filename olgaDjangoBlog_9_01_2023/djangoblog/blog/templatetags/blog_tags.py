...
def load_breadcrumb(article):
    """
    Получить хлебные крошки статьи
    :param article:
    :return:
    """
...
def load_sidebar(user, linktype):
    ...
    if value:
        ...
        # облако тегов рассчитать размер шрифта
        # Рассчитайте среднее значение на основе суммы. Размер (число/среднее) * размер шага
        increment = 5
...
def load_article_metas(article, user):
    """
    Получить метаинформацию статьи
    :param article:
    :return:
    """
    return {
...
def load_pagination_info(page_obj, page_type, tag_name):
    ...
    if page_type == 'Архивы тегов категорий':
        ...
        if page_obj.has_previous():
            ...
            previous_url = reverse(
                ...
                kwargs={
                    'page': previous_number,
                    'tag_name': tag.slug})
    if page_type == 'Архив авторских статей':
        if page_obj.has_next():
            next_number = page_obj.next_page_number()
            next_url = reverse(
...
def load_article_detail(article, isindex, user):
    """
    Загрузить информацию о статье
    :param article:
    :param isindex: Является ли это страницей списка, если это страница списка, отображается только сводка
    :return:
    """
    from djangoblog.utils import get_blog_setting
    blogsetting = get_blog_setting()
